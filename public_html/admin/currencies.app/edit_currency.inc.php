<?php
  ...
  if (isset($_POST['save'])) {
    ...
    try {
      ...
      if ((!empty($_POST['set_store']) || $_POST['code'] == settings::get('site_currency_code')) && (float)$_POST['value'] != 1) {
        throw new Exception(language::translate('error_site_currency_must_have_value_1', 'The store currency must always have the currency value 1.0.'));
      }
      ...
      if (empty($_POST['set_store']) && isset($currency->data['code']) && $currency->data['code'] == settings::get('site_currency_code') && $currency->data['code'] != $_POST['code']) {
        throw new Exception(language::translate('error_cannot_rename_site_currency', 'You must change the store currency before renaming it.'));
      }
      ...
      if (!empty($_POST['set_store']) && empty($_POST['status']) && isset($currency->data['code']) && $currency->data['code'] == settings::get('site_currency_code')) {
        throw new Exception(language::translate('error_cannot_set_disabled_site_currency', 'You cannot set a disabled currency as store currency.'));
      }
      ...
      if (!empty($_POST['set_store'])) {
        database::query("update ". DB_TABLE_SETTINGS ." set `value` = '". database::input($_POST['code']) ."' where `key` = 'site_currency_code' limit 1;");
      }
  ...
  $site_currency = reference::currency(settings::get('site_currency_code'));
?>
    ...
    <?php echo functions::form_draw_form_begin('currency_form', 'post', false, false, 'style="max-width: 640px;"'); ?>
      ...
      <div class="row">
        <div class="form-group col-md-6">
          ...
          <div class="input-group">
            ...
            <span class="input-group-addon"><?php echo $site_currency->code; ?></span>
          </div>
        </div>
      ...
      <div class="row">
        ...
        <div class="form-group col-md-6">
          ...
          <div class="checkbox">
            <label><?php echo functions::form_draw_checkbox('set_store', '1', (isset($currency->data['code']) && $currency->data['code'] && $currency->data['code'] == settings::get('site_currency_code')) ? '1' : true); ?> <?php echo language::translate('description_set_as_site_currency', 'Set as store currency'); ?></label>
          </div>
        </div>
      </div>
