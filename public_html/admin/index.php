<?php
  ...
  document::$template = settings::get('site_template_admin');
...
// Start page
  if (empty($_GET['app'])) {
    ...
    if (settings::get('maintenance_mode')) {
      notices::add('notices', language::translate('reminder_site_in_maintenance_mode', 'The store is in maintenance mode.'));
    }
...
// App content
    ...
    if (empty(user::$data['apps']) || (!empty(user::$data['apps'][$_GET['app']]['status']) && in_array($_GET['doc'], user::$data['apps'][$_GET['app']]['docs']))) {
      ...
      document::$snippets['help_link'] = document::link('https://xmexpi.com/');
