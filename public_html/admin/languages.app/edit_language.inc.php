  ...
  if (isset($_POST['save'])) {
    ...
    try {
      ...
      if (empty($_POST['set_store']) && isset($language->data['code']) && $language->data['code'] == settings::get('site_language_code') && $language->data['code'] != $_POST['code']) {
        throw new Exception(language::translate('error_cannot_rename_site_language', 'You must change the store language before renaming it.'));
      }
      ...
      if (!empty($_POST['set_store']) && empty($_POST['status']) && isset($language->data['code']) && $language->data['code'] == settings::get('site_language_code')) {
        throw new Exception(language::translate('error_cannot_set_disabled_site_language', 'You cannot set a disabled language as store language.'));
      }
      ...
      if (!empty($_POST['set_store'])) {
        database::query("update ". DB_TABLE_SETTINGS ." set `value` = '". database::input($_POST['code']) ."' where `key` = 'site_language_code' limit 1;");
      }
...
?>
      ...
      <div class="row">
        <div class="form-group col-md-6">
          ...
          <div class="radio">
            <label><?php echo functions::form_draw_checkbox('set_store', '1', (isset($language->data['code']) && $language->data['code'] && $language->data['code'] == settings::get('site_language_code')) ? '1' : true); ?> <?php echo language::translate('description_set_as_site_language', 'Set as store language'); ?></label>
          </div>
        </div>
      </div>
