<?php
  ...
  if (isset($_POST['save'])) {
    ...
    try {
      if (!preg_match('#(\.site)$#', $_POST['template_site'])) throw new Exception(language::translate('error_invalid_site_template', 'Not a valid site template'));
      ...
      if ($_POST['template_site'] != settings::get('site_template_select')) {
        database::query(
          "update ". DB_TABLE_SETTINGS ."
          set
            `value` = '". database::input($_POST['template_site']) ."',
            date_updated = '". date('Y-m-d H:i:s') ."'
          where `key` = '". database::input('site_template_select') ."'
          limit 1;"
        );

      // Load template settings structure
        $template_config = include vmod::check(FS_DIR_APP . 'includes/templates/' . basename($_POST['template_site']) .'/config.inc.php');
        if (!is_array($template_config)) include vmod::check(FS_DIR_APP . 'includes/templates/' . basename($_POST['template_site']) .'/config.inc.php'); // Backwards compatibility
      ...
      // Set template default settings
        ...
        database::query(
          ...
          set
            ...
            date_updated = '". date('Y-m-d H:i:s') ."'
          where `key` = '". database::input('site_template_site_settings') ."'
          limit 1;"
        );
      ...
      if ($_POST['template_admin'] != settings::get('site_template_admin')) {
        database::query(
          "update ". DB_TABLE_SETTINGS ."
          set
            `value` = '". database::input($_POST['template_admin']) ."',
            date_updated = '". date('Y-m-d H:i:s') ."'
          where `key` = '". database::input('site_template_admin') ."'
          limit 1;"
        );
      }
    ...
    <?php echo functions::form_draw_form_begin('template_form', 'post', null, false, 'style="max-width: 320px;"'); ?>
      <div class="form-group">
        <label><?php echo language::translate('title_catalog_template', 'Catalog Template'); ?></label>
          <div class="input-group">
            <?php echo functions::form_draw_templates_list('site', 'template_site', empty($_POST['template_site']) ? settings::get('site_template_site') : true); ?>
      ...
      <div class="form-group">
        <label><?php echo language::translate('title_admin_template', 'Admin Template'); ?></label>
          <?php echo functions::form_draw_templates_list('admin', 'template_admin', empty($_POST['template_admin']) ? settings::get('site_template_admin') : true); ?>
      </div>
      <div class="btn-group">
