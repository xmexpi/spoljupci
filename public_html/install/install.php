...
if (php_sapi_name() == 'cli') {
  ...
  if ((!isset($argv[1])) || ($argv[1] == 'help') || ($argv[1] == '-h') || ($argv[1] == '--help')) {
    echo "\nxMexpi® 1.0\n"
      ...
      . "  --db_table_prefix    Set database table prefix (Default: xp_).\n"
...
try {
  ...
  echo '<h1>xMexpi Installer</h1>' . PHP_EOL . PHP_EOL;
  ...
  if (empty($_REQUEST['timezone']) && !empty($_REQUEST['site_time_zone'])) {
    $_REQUEST['timezone'] = $_REQUEST['site_time_zone']; // Backwards compatible
  ...
  $sql = str_replace('`xp_', '`' . $_REQUEST['db_table_prefix'], $sql);
  ...
  $map = array(
    ...
    '{SITE_CONFIGURATION}' => $post_data['hash_key'],
  ...
  database::query(
    "insert into " . str_replace('`xp_', '`' . $_REQUEST['db_table_prefix'], '`xp_users`') . "
  ...
  if (defined('PLATFORM_VERSION')) {
    ...
    database::query(
      "update " . str_replace('`xp_', '`' . $_REQUEST['db_table_prefix'], '`xp_settings`') . "
  ...
  if (!empty($_REQUEST['development_type']) && $_REQUEST['development_type'] == 'advanced') {
    ...
    $files_to_delete = array(
      '../includes/templates/default.site/css/app.css',
      '../includes/templates/default.site/css/checkout.css',
      '../includes/templates/default.site/css/framework.css',
      '../includes/templates/default.site/css/printable.css',
    );
    ...
    $files_to_delete = array(
      '../includes/templates/default.site/css/*.min.css',
      '../includes/templates/default.site/css/*.min.css.map',
      '../includes/templates/default.site/js/*.min.js',
      '../includes/templates/default.site/js/*.min.js.map',
      '../includes/templates/default.site/less/',
    );
    ...
    foreach (glob('../includes/templates/default.site/layouts/*.inc.php') as $file) {
