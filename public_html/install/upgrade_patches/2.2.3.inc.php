...
// Delete files
  $deleted_files = array(
    FS_DIR_APP . 'includes/templates/default.site/pages/page.inc.php',
  );
...
// Cannot copy, so migrate option groups into attribute groups
    ...
    $site_language_code = database::fetch($setting_query, 'value');

    $option_groups_query = database::query(
      "select og.*, ogi.name from ". DB_TABLE_PREFIX ."option_groups og
      left join ". DB_TABLE_PREFIX ."option_groups_info ogi on (ogi.group_id = og.id and language_code = '". database::input($site_language_code) ."')
      order by id;"
    );
    ...
    // Is there an attribute group matching the option group name?
      $attribute_groups_query = database::query(
        ...
        left join ". DB_TABLE_PREFIX ."attribute_groups_info agi on (agi.group_id = ag.id and agi.language_code = '". database::input($site_language_code) ."')
    ...
    // Update values in products_options and products_options_values
      $option_values_query = database::query(
        ...
        left join ". DB_TABLE_PREFIX ."option_values_info ovi on (ovi.value_id = ov.id and ovi.language_code = '". database::input($site_language_code) ."')
      ...
      // Is there an attribute value matching the option value name?
        $attribute_values_query = database::query(
          ...
          left join ". DB_TABLE_PREFIX ."attribute_values_info agvi on (agvi.value_id = agv.id and agvi.language_code = '". database::input($site_language_code) ."')
