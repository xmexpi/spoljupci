...
-- --------------------------------------------------------
ALTER TABLE `xp_translations` ADD `text_nb` text NOT NULL AFTER `text_en`;
-- --------------------------------------------------------
INSERT INTO `xp_currencies` (`status`, `code`, `number`, `name`, `value`, `decimals`, `prefix`, `suffix`, `priority`, `date_updated`, `date_created`) VALUES
(1, 'NOK', '578', 'Norske kroner', 1.00, 2, 'kr. ', '', 0, NOW(), NOW());
-- --------------------------------------------------------
ALTER TABLE `xp_products_prices` ADD `NOK` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
ALTER TABLE `xp_products_campaigns` ADD `NOK` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
ALTER TABLE `xp_products_options_values` ADD `NOK` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
UPDATE `xp_settings` SET `value` = 'NOK' WHERE `key` in ('site_currency_code', 'default_currency_code');
-- --------------------------------------------------------
UPDATE `xp_currencies` SET `value` = 8.0364 WHERE `code` = 'USD' LIMIT 1;
-- --------------------------------------------------------
UPDATE `xp_currencies` SET `value` = 9.4268 WHERE `code` = 'EUR' LIMIT 1;
-- --------------------------------------------------------
