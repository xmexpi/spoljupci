...
-- --------------------------------------------------------
ALTER TABLE `xp_translations` ADD `text_sv` text NOT NULL AFTER `text_en`;
-- --------------------------------------------------------
...
-- --------------------------------------------------------
ALTER TABLE `xp_products_prices` ADD `SEK` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
ALTER TABLE `xp_products_campaigns` ADD `SEK` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
ALTER TABLE `xp_products_options_values` ADD `SEK` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
UPDATE `xp_settings` SET `value` = 'SEK' WHERE `key` in ('site_currency_code', 'default_currency_code');
-- --------------------------------------------------------
UPDATE `xp_currencies` SET `value` = 9.76 WHERE `code` = 'USD' LIMIT 1;
-- --------------------------------------------------------
UPDATE `xp_currencies` SET `value` = 10.68 WHERE `code` = 'EUR' LIMIT 1;
-- --------------------------------------------------------
