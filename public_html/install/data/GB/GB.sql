...
-- --------------------------------------------------------
ALTER TABLE `xp_products_prices` ADD `GBP` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
ALTER TABLE `xp_products_campaigns` ADD `GBP` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
ALTER TABLE `xp_products_options_values` ADD `GBP` DECIMAL(11,4) NOT NULL;
-- --------------------------------------------------------
UPDATE `xp_settings` SET `value` = 'GBP' WHERE `key` in ('site_currency_code', 'default_currency_code');
-- --------------------------------------------------------
UPDATE `xp_currencies` SET `value` = 0.7475 WHERE `code` = 'USD' LIMIT 1;
-- --------------------------------------------------------
UPDATE `xp_currencies` SET `value` = 0.8768 WHERE `code` = 'EUR' LIMIT 1;
-- --------------------------------------------------------
