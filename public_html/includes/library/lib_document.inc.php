  ...
  class document {
    ...
    // Set template
      if (preg_match('#^('. preg_quote(WS_DIR_ADMIN, '#') .')#', parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH))) {
        self::$template = settings::get('site_template_admin');
      } else {
        self::$template = settings::get('site_template_site');
      }
    ...
    // Set some snippets
      ...
      self::$snippets['title'] = array(settings::get('site_name'));
    ...
    // Hreflang
      if (!empty(route::$route['page'])) {
        ...
        foreach (language::$languages as $language) {
          ...
          self::$snippets['head_tags']['hreflang'] .= '<link rel="alternate" hreflang="'. $language['code'] .'" href="'. document::href_ilink(route::$route['page'], array(), true, array(), $language['code']) .'" />' . PHP_EOL;
        }
        self::$snippets['head_tags']['hreflang'] = trim(self::$snippets['head_tags']['hreflang']);
      }

    // Get template settings
      $template_config = include vmod::check(FS_DIR_APP .'includes/templates/'. settings::get('site_template_site') .'/config.inc.php');
      if (!is_array($template_config)) include vmod::check(FS_DIR_APP .'includes/templates/'. settings::get('site_template_site') .'/config.inc.php'); // Backwards compatibility

      self::$settings = settings::get('site_template_site_settings') ? json_decode(settings::get('site_template_site_settings'), true) : array();
